...
export class HydeService {
    ...
    public async generateHypotheticalDocument(
        query: string,
        config: HydeConfig
    ): Promise<string> {

        const builtPrompt: string = this.buildHydePrompt(query, config.domain, config.documentType);
    ...
    public async getHypotheticalDocumentEmbedding(hypotheticalDoc: string) {
